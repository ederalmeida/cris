        ...
        for i in range(0, len(relacao_xml)):
            ...
            if type(xml.nfeProc.NFe.infNFe.det) == unt.Element:
                NFes[i].itens = [[1,
                                ...
                                str(round(float(xml.nfeProc.NFe.infNFe.det.prod.vUnCom.cdata))).replace('.',','),
                                str(round(float(xml.nfeProc.NFe.infNFe.det.prod.vProd.cdata))).replace('.',',')]]
            ...
            if type(xml.nfeProc.NFe.infNFe.det) == list:
                ...
                for n in range(0, len(xml.nfeProc.NFe.infNFe.det)):
                    itens.append([l+1,
                                ...
                                str(round(float(xml.nfeProc.NFe.infNFe.det[l].prod.vUnCom.cdata))).replace('.',','),
                                str(round(float(xml.nfeProc.NFe.infNFe.det[l].prod.vProd.cdata))).replace('.',',')])
                    l += 1
                NFes[i].itens = itens
