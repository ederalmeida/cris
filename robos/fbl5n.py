...
def executar_robo(informacoes_janela_fbl5n):
    ...
    # Abrindo a transação FBL5n
    sap.session.findById('wnd[0]').iconify()
    sap.session.findById('wnd[0]').maximize()
    sap.session.findById('wnd[0]/tbar[0]/okcd').text = 'FBL5N'
    sap.session.findById('wnd[0]').sendVKey(0)

    # Verificando se o usuário tem acesso a transação desejada
    if sap.session.findById('wnd[0]/sbar').text == "Sem autorização para a transação FBL5N":
        sg.popup("Você não tem acesso a transação FBL5N. A execução do robô será cancelada")
        exit()
    ...
    for conta in contas_conciliaveis:
        ...
        sap.session.findById("wnd[0]/usr/ctxtDD_KUNNR-LOW").text = conta
        sap.session.findById("wnd[0]/usr/ctxtDD_BUKRS-LOW").text = company_code
        ...
        if sap.session.findById('wnd[0]/sbar').text == 'Nenhuma partida selecionada (ver texto descritivo)' or\
            ...
            screenParametrizacao.save(vcs.winapi_path(caminho_pasta_salvar_ipes) + '\\prints\\' + mes_referencia + ' - ' + conta +  ' - ' + \
                data_execucao + ' - FBL5N - ' + tipo_de_partidas + ' - 01 parametrizacao.jpg')
            screenExecucao.save(vcs.winapi_path(caminho_pasta_salvar_ipes) + '\\prints\\' + mes_referencia + ' - ' + conta +  ' - ' + \
                data_execucao + ' - FBL5N - ' + tipo_de_partidas + ' - 02 resultados.jpg')
            continue
        ...
        + data_execucao + ' - FBL5N - ' + tipo_de_partidas + '.xlsx')
