...
class OutroEquipamento(SGPMRBase):
    def run(self):
        resultado_arquivo = self.EXCEL_FOLDER + 'sgpm_extrapatrimonial_reprocessamento_resultado.csv'
        df_dados = self.get_excel_file(filename='sgpmr_carga_reprocessamento.xlsx')
        ...
        for _, dados in df_dados.iterrows():
            ...
            porte = dados['Porte do Equipamento']
            modulo = dados['Módulo']
            modulo = modulo.strip()
            ...
            try:
                fd_tensao = self.driver.find_element_by_xpath(
            ...
            try:
                fd_porte = self.driver.find_element_by_xpath(
                    '/html/body/div[3]/div[2]/form/div/div/div[2]/div[2]/div/div/div[1]/div[5]/div[1]/div/div[1]/span')
                fd_porte.click()
                time.sleep(2)
                fd_porte_resp = self.driver.find_element_by_xpath(r"//li[text()='" + porte + "']")
                fd_porte_resp.click()
            except:
                print(f'ERRO EM {seq}')
                temp = pd.DataFrame(columns=df_dados.columns, data=[dados])
                temp['SGPMR_CODIGOS'] = ['Erro em porte']
                df_dados_2 = df_dados_2.append(temp)
                df_dados_2.to_csv(resultado_arquivo, index=False)
                continue
            ...
            print(f'Execucao: {tempo} segundos')
